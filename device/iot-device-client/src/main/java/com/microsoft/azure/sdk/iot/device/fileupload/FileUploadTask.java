...
public final class FileUploadTask implements Runnable
{
    ...
    public void run()
    {
        ...
        {
            /* Codes_SRS_FILEUPLOADTASK_21_031: [If run failed to send the request, it shall call the userCallback with the stratus `ERROR`, and abort the upload.] */
    ...
    private IotHubStatusCode getContainer() throws IOException, IllegalArgumentException, URISyntaxException
    {
        ...
        {
            /* Codes_SRS_FILEUPLOADTASK_21_014: [If result status for the blob request is `OK_EMPTY`, the run shall call the userCallback with the stratus `BAD_FORMAT`, and abort the upload.] */
            resultStatus = IotHubStatusCode.BAD_FORMAT;
        }
    ...
    private IotHubStatusCode sendNotification(FileUploadStatusParser fileUploadStatusParser)
    {
        ...
        try
        {
            ...
            message.setIotHubMethod(IotHubMethod.POST);
            /* Codes_SRS_FILEUPLOADTASK_21_025: [The run shall set the message URI path as `/notifications`.] */
